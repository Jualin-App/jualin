...
export const productService = {
  async fetchAll(params = {}) {
    ...
    if (res?.products) {
      return {
        ...
        totalProducts: res.totalProducts ?? res.total ?? res.products.length,
        totalPages: res.totalPages ?? res.last_page ?? 1,
        currentPage: res.currentPage ?? res.current_page ?? 1,
      };
    }
  ...
  async create(productData, imageFile = null) {
    const storedUser =
      typeof window !== "undefined"
        ...
        : null;
    const sellerId =
      storedUser?.id || storedUser?.user_id || storedUser?.userId;
  ...
  async update(id, productData, imageFile = null) {
    ...
    const res = await fetcher.patch(`/api/v1/products/${id}`, productData);
    return normalizeProduct(res?.data || res);
  },
  async delete(id) {
